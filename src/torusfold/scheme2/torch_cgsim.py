...
def cg_energy_forces(pos_nm, pairs_ij, pair_w=None, lam=1.0,
    ...
    with torch.enable_grad():
        ...
        # replica 0 was dropped from replica 3's GB interaction entirely.
    ...
    with torch.enable_grad():
        ...
        if gb_pos.grad is not None:
            ...
            #
            # Removed rather than retuned, for the reason above: the excluded volume was
            # rebuilt with sigma = 0.3975 nm and diverges as d -> 0, so the GB singularity the
            # clip guarded against is no longer reachable. The global force cap and
            # _require_finite below are untouched. If that is wrong, the honest repair is a
            # soft-core GB pair term whose energy and gradient are clipped together, not a clip
            # on the gradient alone -- which is what this was, and it made the returned force
            # stop being -dE/dx exactly where the energy was largest.
            gb_f = -gb_pos.grad
    ...
    def run(self, coords_A, pairs, n_steps, verbose=True):
        ...
        for rep in range(n_reports):
            ...
            for ri in range(self.n_replicas - 1):
                ...
                # expo IS the Metropolis exponent, so accept = min(1, exp(expo)). exp(-expo)
                # is the reciprocal and inverts the exchange direction. Same defect as
                # rest2_remd_2d.py:123 and rest2_sampler.py:163.
...
class BatchedREMD2D:
    ...
    def run(self, coords_A, pairs, n_steps, verbose=True, initial_pos_3bead=None,
        ...
        # Excluded-volume cell list, built HERE rather than down in the MD loop.
        #
        # _energy_split's three energies and the 500-step relaxation below used to call
        # cg_energy_forces without a cell_list, so the excluded volume was absent from the
        # exchange criterion and from the relaxation forces while the main loop had it.
        # The dynamics ran on a field the swap decision could not see.
        #
        # Measured on a real 24-replica configuration (results/remd_ckpt_idx0.npz): with and
        # without the cell list the force differs by up to 753.7 kJ/mol/nm -- 21.2 percent of
        # the maximum force -- and the per-replica energy by 19.4 kJ/mol.
        #
